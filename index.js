...
// ============ KONEKSI MONGODB ============
// URI yang sudah disesuaikan dengan data MongoDB Atlas Anda
...
const ReportSchema = new mongoose.Schema({
  type: { type: String, default: "daily" }, // Untuk membedakan jenis laporan jika nanti berkembang
  daily: [{
    id: Number,
    date: String,
    title: String,
...
// Helper untuk memastikan dokumen reports global sudah ada di MongoDB
async function initReportDoc() {
  let reportDoc = await ReportModel.findOne({ type: "daily" });
  if (!reportDoc) {
    reportDoc = new ReportModel({ type: "daily", daily: [] });
...
async function checkRunningPositions() {
  ...
  // Load langsung dari MongoDB
  const runningSignals = await SignalModel.find({ status: "RUNNING" });
  const today = moment().tz('Asia/Jakarta').format('YYYY-MM-DD');

  for (const s of runningSignals) {
    ...
    if (newStatus) {
      ...
      // Update report setelah ada posisi close
      await updateAllReports();
    } else {
      // Jika harga High/Low baru berubah namun status masih RUNNING, tetap simpan perubahannya
      await s.save();
    }
  }
}
...
async function updateAllReports() {
  try {
    ...
    const existingDailyIdx = reportDoc.daily.findIndex(
      (r) => r.title === dailyEntry.title,
    );
    
    if (existingDailyIdx !== -1) {
      reportDoc.daily[existingDailyIdx] = dailyEntry;
    } else {
      reportDoc.daily.unshift(dailyEntry);
      if (reportDoc.daily.length > 30) reportDoc.daily.pop();
    }

    await reportDoc.save();
    console.log("[REPORT] Daily report updated in MongoDB.");
  } catch (err) {
...
(async () => {
  ...
  teleClient.addEventHandler(async (update) => {
    ...
    if (newSignal) {
      ...
      // Cek ke MongoDB apakah ada kode saham yang berstatus RUNNING
      const existingSignal = await SignalModel.findOne({ stockCode: newSignal.stockCode, status: "RUNNING" });
      
      if (existingSignal) {
        // Ganti data sinyal lama dengan yang baru di MongoDB
        await SignalModel.replaceOne({ _id: existingSignal._id }, newSignal);
        console.log(`[SIGNAL] Replace signal for ${newSignal.stockCode} with new data in MongoDB.`);
      } else {
        // Tambahkan data baru ke MongoDB
        const signalDoc = new SignalModel(newSignal);
        await signalDoc.save();
        console.log(`[SIGNAL] New signal added to MongoDB: ${newSignal.stockCode}`);
      }
      
      // Update Laporan secara otomatis setelah data masuk
      await updateAllReports();
    } else {
